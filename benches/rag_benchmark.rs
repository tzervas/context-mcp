...
use context_mcp::{
    Context, ContextDomain, ContextStore, StorageConfig,
    rag::{RagProcessor, RagConfig, RetrievalQuery},
};
...
fn rag_benchmarks(c: &mut Criterion) {
    ...
    c.bench_function("parallel_rag_queries", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            let store = ContextStore::new(config).unwrap();
            let rag = RagProcessor::with_config(
                store.clone(),
                RagConfig {
                    max_results: 10,
            ...
            let queries = vec![
                RetrievalQuery::new().with_text("Content 1"),
                RetrievalQuery::new().with_text("Content 2"),
                RetrievalQuery::new().with_text("Content 3"),
            ];
